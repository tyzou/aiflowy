...
![](/assets/images/locale.png)

## 配置默认语言

只需要覆盖默认的偏好设置即可，在对应的应用内，找到 `app/src/preferences.ts` 文件，修改 `locale` 的值即可：
...
- 请不要将业务翻译文本放到 `@aiflowy/locales` 内，这样可以更好的管理业务和通用的翻译文本。
- 有多个语言包的情况下，新增翻译文本时，需要在所有语言包内新增对应的文本。

:::

新增翻译文本，只需要在对应的应用内，找到 `app/src/locales/langs/`，新增对应的文本即可，例:

**app/src/locales/langs/zh-CN/\*.json**

````ts
```json
{
  "about": {
    "desc": "AIflowy Admin 是一个现代的管理模版。"
  }
}
````
...
{
  "about": {
    "desc": "AIflowy Admin is a modern management template."
  }
}
````
...
通过 `@aiflowy/locales`，你可以很方便的使用翻译文本：
...
- 在对应的应用内，找到 `app/src/locales/langs` 文件，新增对应的语言包 `zh-TW.json`
...
如果你想关闭界面上的语言切换显示按钮，在对应的应用内，找到 `app/src/preferences.ts` 文件，修改 `locale` 的值即可：
...
每个应用都有一个独立的语言包，它可以覆盖通用的语言配置，你可以通过远程加载的方式来获取对应的语言包，只需要在对应的应用内，找到 `app/src/locales/index.ts` 文件，修改 `loadMessages` 方法即可：
...
不同应用内使用的第三方组件库或者插件国际化方式可能不一致，所以需要差别处理。 如果你需要引入第三方的语言包，你可以在对应的应用内，找到 `app/src/locales/index.ts` 文件，修改 `loadThirdPartyMessage` 方法即可：
...
- 关闭 `vue-i18n`的警告提示，在`app/src/locales/index.ts`文件内，修改`missingWarn`为`false`即可：
